...
-- 遵循 建表规范.md: BIGINT 主键, 审计字段 (create_user, create_time, update_user, update_time, del_flag)
-- ----------------------------
...
CREATE TABLE `sys_dict_data` (
  ...
  `dict_type`     VARCHAR(100) NOT NULL COMMENT '字典类型 (关联 sys_dict_type)',
  `css_class`     VARCHAR(100) DEFAULT NULL COMMENT '样式属性 (其他样式)',
  `list_class`    VARCHAR(100) DEFAULT NULL COMMENT '表格回显样式 (success/info/warning/danger)',
...
CREATE TABLE `sys_file` (
  ...
  `store_type`    VARCHAR(20)  DEFAULT 'local' COMMENT '存储方案 (local/oss/minio)',
  `create_user`   BIGINT       DEFAULT NULL COMMENT '创建人ID',
  `create_time`   DATETIME     DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_user`   BIGINT       DEFAULT NULL COMMENT '更新人ID',
  `update_time`   DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
